...
ALSASTATE_INIT_DIR = "${localstatedir}"
ALSASTATE_DIR = "${localstatedir}"

do_install() {
    # Only install the init script when 'sysvinit' is in DISTRO_FEATURES.
    if ${@bb.utils.contains('DISTRO_FEATURES','sysvinit','true','false',d)}; then
	sed -i -e "s:#STATEINITDIR#:${ALSASTATE_INIT_DIR}/lib/alsa:g" ${WORKDIR}/alsa-state-init
	sed -i -e "s:#STATEDIR#:${ALSASTATE_DIR}/lib/alsa:g" ${WORKDIR}/alsa-state-init
	install -d ${D}${sysconfdir}/init.d
	install -m 0755 ${WORKDIR}/alsa-state-init ${D}${sysconfdir}/init.d/alsa-state
    fi

    install -d ${D}/${ALSASTATE_INIT_DIR}/lib/alsa
    install -d ${D}${sysconfdir}
    install -m 0644 ${WORKDIR}/asound.conf ${D}${sysconfdir}
    install -m 0644 ${WORKDIR}/*.state ${D}${ALSASTATE_INIT_DIR}/lib/alsa
}
...
FILES_alsa-states = "${ALSASTATE_INIT_DIR}/lib/alsa/*.state"
...
pkg_postinst_${PN}() {
	...
	then
		...
		then
			${sbindir}/alsactl -f ${ALSASTATE_INIT_DIR}/lib/alsa/asound.state restore
		fi
	fi
}
